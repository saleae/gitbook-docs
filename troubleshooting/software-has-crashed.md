...
* If the Logic software runs fine while the Logic hardware is not connected, but crashes immediately after the Logic hardware is connected to the PC, please follow the troubleshooting steps below. Please follow the steps described in that support article to find your crash log, open the crash log, and to look for the specified error function in the header of the crash log.
